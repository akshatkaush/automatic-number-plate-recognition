...
def get_lprnet_preds(images, args):
    lprnet = build_lprnet(lpr_max_len=15, class_num=len(CHARS), dropout_rate=0)
    device = torch.device("cuda:0" if args.cuda else "cpu")
    lprnet.to(device)
    flag=True
    if flag:
        lprnet.load_state_dict(torch.load(args.lpr_weights))
    ...
    return Greedy_Decode_Eval(lprnet, images, args)

def Greedy_Decode_Eval(Net, images, args):
    if args.cuda:
