...
def preloader(is_cuda):
    ...
    if(is_cuda):
        print(torch.load(os.path.join(f"{current_path}/data", data_name + ".pth"),map_location=torch.device('cuda')))
    else:
    ...
    if(is_cuda):
        print(torch.load(seg_weights,map_location=torch.device('cuda')).keys())
        model.load_state_dict(torch.load(seg_weights,map_location=torch.device('cuda'))["state_dict"])
    else:
